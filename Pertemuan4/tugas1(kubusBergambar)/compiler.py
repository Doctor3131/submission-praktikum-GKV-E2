...
def main():
    # Compile source files
    subprocess.call(f"g++ -c main.cpp -I {include_dir} {preprocessor} -o main.o", shell=True)
    subprocess.call(f"g++ -c imageloader.cpp -I {include_dir} {preprocessor} -o imageloader.o", shell=True)
    subprocess.call(f"g++ -c glad.c -I {include_dir} {preprocessor} -o glad.o", shell=True)

    # Link object files
    subprocess.call(f"g++ main.o imageloader.o glad.o -o {file_name}.exe -L {lib_dir} {lib} -Wl,--allow-multiple-definition", shell=True)

    # shutil.copy("sample_640x426.bmp", ".")
